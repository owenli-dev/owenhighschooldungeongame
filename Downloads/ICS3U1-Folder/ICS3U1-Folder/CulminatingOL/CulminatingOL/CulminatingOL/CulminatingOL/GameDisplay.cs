...
namespace CulminatingOL
{
    ...
    {
        static int intUserCoins = 0; //The amount of coins the user has earned from dungeons
        ...
        public GameDisplay(int intCoinsEarned, int intCurrentTotal = -1)
        {
            InitializeComponent();
            this.pnlDifficulty.Visible = false; //difficulty panel hidden
            this.pnlLevelSelect.Visible = true; //level select panel shown

            if (intCurrentTotal != -1) //If a current total is provided, sync it
            {
                intUserCoins = intCurrentTotal; //Set static coins to current total
            }

            intUserCoins += intCoinsEarned; //Add earned coins to static total
        ...
        public GameDisplay()
        {
            ...
            this.pnlLevelSelect.Visible = true; //level select panel shown
            this.lblCoinAmount.Text = intUserCoins.ToString(); //Displays the number of coins the user has
        } //no new coins if the person just decides to go back with no action
